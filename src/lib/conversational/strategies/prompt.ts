 ...
 *
 * Defines pluggable prompt generation strategies for conversational forms.
 * Different templates can implement their own prompt strategies while
...
/**
 * Topic guidance result
 */
export interface TopicGuidance {
  /** Topic to explore next (if any) */
 ...
 * Prompt Strategy Interface
 *
 * Templates implement this interface to customize prompt generation.
 */
export interface PromptStrategy {
  /** Strategy identifier (for debugging/logging) */
  readonly id: string;
   ...
   */
  buildSystemPrompt(config: ConversationalFormConfig): string;

  /**
   * Build conversation context for ongoing conversations
   * @param state Current conversation state
   * @param config Form configuration
   * @returns Context prompt with progress and next steps
   */
  ...
  /**
   * Get guidance for the next topic to explore
   * @param state Current conversation state
   * @param config Form configuration
   * @returns Topic and guidance for next question
   */
   ...
   * @param state Current conversation state
   * @param config Form configuration
   * @returns Prompt for summarizing and completing
   */
  buildWrapUpPrompt(
    state: ConversationState,
    config: ConversationalFormConfig
   ...
   * @param config Form configuration
   * @returns Guidance for extracting topic data
   */
 ...
 *
 * Single source of truth for persona styles.
 * Used by DefaultPromptStrategy and available to custom strategies.
 */
...
/**
 * Build persona section from persona config
 * Utility function available to all prompt strategies
 */
export function buildPersonaSection(persona: ConversationPersona): string {
  if (persona.style === 'custom' && persona.customPrompt) {
    return persona.customPrompt;
  }

  let section = PERSONA_STYLES[persona.style] || PERSONA_STYLES.friendly;
 ...
 *
 * Generic prompt strategy that works for any conversational form.
 * Templates can extend this or implement their own strategy.
 */
export class DefaultPromptStrategy implements PromptStrategy {
  ...
  buildSystemPrompt(config: ConversationalFormConfig): string {
    ...
    return `You are a helpful AI assistant conducting a conversation to gather information for a form.
...
${
  uncoveredRequiredTopics.length > 0
    ? `**Required (must cover):**\n${uncoveredRequiredTopics
        .map(
          (t) =>
            `- ${t.name}: ${config.topics.find((ct) => ct.id === t.topicId)?.description || ''}`
        )
        .join('\n')}`
    : ''
}
  ...
  getNextTopicGuidance(
    ...
    if (uncoveredRequired) {
      const topicConfig = config.topics.find(
        (t) => t.id === uncoveredRequired.topicId
      );
      if (topicConfig) {
        return {
          topic: topicConfig,
    ...
    if (uncoveredImportant) {
      const topicConfig = config.topics.find(
        (t) => t.id === uncoveredImportant.topicId
      );
      if (topicConfig) {
        return {
          topic: topicConfig,
    ...
    return {
      guidance:
        "All required and important topics are covered. Summarize what you've learned and confirm if the user has anything else to add before completing.",
    };
  }
...
/**
 * IT Helpdesk Prompt Strategy
 *
 * Specialized prompt strategy for IT support ticket intake.
 */
export class ITHelpdeskPromptStrategy implements PromptStrategy {
  readonly id = 'it-helpdesk';

  buildSystemPrompt(_config: ConversationalFormConfig): string {
...
6. Summarize and confirm`;
  ...
  buildConversationContext(
    ...
    // Use default implementation with IT-specific additions
    const defaultStrategy = new DefaultPromptStrategy();
    const baseContext = defaultStrategy.buildConversationContext(state, config);

    // Add IT-specific context
    return `${baseContext}

### IT Helpdesk Specific
- If the user mentioned a device, note the type (laptop, desktop, phone, etc.)
- If they mentioned an application, note the name
- Watch for implicit urgency cues (deadline mentions, "blocking work", "critical")
- Hardware issues with lost/stolen devices need security follow-up`;
  }
  ...
  getNextTopicGuidance(
    ...
    const defaultStrategy = new DefaultPromptStrategy();
    return defaultStrategy.getNextTopicGuidance(state, config);
  }
  ...
  buildWrapUpPrompt(
    ...
    return `You have gathered all required information for the IT support ticket. Provide a clear summary that includes:
1. Issue category and type
2. Urgency level
3. Brief description of the issue
4. Contact preference

Confirm these details with the user and let them know their ticket has been submitted and will be assigned to the appropriate team.`;
  }
  ...
  buildExtractionGuidance(
    ...
    // Use default implementation
    const defaultStrategy = new DefaultPromptStrategy();
    return defaultStrategy.buildExtractionGuidance(topic, config);
  }
}
 ...
 */
export function getPromptStrategy(id: string): PromptStrategy {
  switch (id) {
    case 'it-helpdesk':
...
/**
 * Create a custom prompt strategy with overrides
 */
export function createCustomPromptStrategy(
  base: PromptStrategy,
  overrides: Partial<PromptStrategy>
): PromptStrategy {
  return {
    id: overrides.id || `${base.id}-custom`,
    buildSystemPrompt:
      overrides.buildSystemPrompt?.bind(overrides) ||
      base.buildSystemPrompt.bind(base),
    buildConversationContext:
      overrides.buildConversationContext?.bind(overrides) ||
      base.buildConversationContext.bind(base),
    getNextTopicGuidance:
      overrides.getNextTopicGuidance?.bind(overrides) ||
      base.getNextTopicGuidance.bind(base),
    buildWrapUpPrompt:
      overrides.buildWrapUpPrompt?.bind(overrides) ||
      base.buildWrapUpPrompt.bind(base),
    buildExtractionGuidance:
      overrides.buildExtractionGuidance?.bind(overrides) ||
      base.buildExtractionGuidance.bind(base),
  };
}
