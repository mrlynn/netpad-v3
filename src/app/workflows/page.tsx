...
export default function WorkflowsRedirect() {
  ...
  useEffect(() => {
    ...
    if (storedProjectId) {
      router.replace(getOrgProjectUrl(organization.orgId, storedProjectId, 'workflows'));
    } else {
      fetch(`/api/projects?orgId=${organization.orgId}`)
        ...
        .then(data => {
          ...
          if (projects.length > 0) {
            ...
            router.replace(getOrgProjectUrl(organization.orgId, defaultProject.projectId, 'workflows'));
          } else {
