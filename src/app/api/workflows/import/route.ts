 ...
 *
 * Handles workflow imports from ChatGPT, Claude MCP, and other sources.
...
import {
  ...
  type CreateWorkflowImportInput,
} from '@/lib/platform/workflowImportsDbOps';

// ============================================
// Constants
// ============================================

const MAX_CONFIG_SIZE_BYTES = 500 * 1024; // 500KB
 ...
 */
function validateSource(source: unknown): WorkflowImportSource {
  const validSources: WorkflowImportSource[] = ['claude-mcp', 'chatgpt', 'cli', 'api', 'share'];
  if (typeof source === 'string' && validSources.includes(source as WorkflowImportSource)) {
    return source as WorkflowImportSource;
  }
  return 'api'; // Default
}
 ...
 *
 * Query parameters:
 * - config: base64-encoded JSON workflow configuration (required)
...
export async function GET(request: NextRequest) {
  try {
    ...
    const input: CreateWorkflowImportInput = {
    ...
    const result = await createWorkflowImport(input);

    // Redirect to import page
    const baseUrl = getBaseUrl();
    const redirectUrl = `${baseUrl}/import/workflow/${result.importId}`;

    return NextResponse.redirect(redirectUrl, { status: 302 });
  } catch (error) {
    console.error('[Workflow Import] GET error:', error);
    return NextResponse.json(
      {
 ...
 * Request body:
 * {
 *   config: WorkflowImportConfig (required)
 *   projectId: string (optional)
 *   source: "claude-mcp" | "chatgpt" | "cli" | "api" | "share" (optional)
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[Workflow Import] POST error:', error);
    return NextResponse.json(
      {
