/**
 * AI Application Generation API Endpoint
 *
 * POST /api/ai/generate-application
 * Generates a complete application (metadata, form, and workflow) from a natural language description.
 */

import { NextRequest, NextResponse } from 'next/server';
import { createApplicationGenerator } from '@/lib/ai/applicationGenerator';
import { GenerateApplicationRequest } from '@/lib/ai/types';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const generationRequest: GenerateApplicationRequest = {
      prompt: body.prompt,
      context: body.context,
      options: body.options,
    };

    const result = await generator.generateApplication(generationRequest);
    ...
    console.error('Application generation API error:', error);
    return NextResponse.json(
      {
        success: false,
